...
export async function createTokens(
  ...
  try {
    ...
    const jwtAccessTokenData = {
      data: { jti, fingerprint, expires: new Date(die) }
    };
    const jwtRefreshTokenData = {
      data: {
        jti: refresh_token,
        fingerprint,
    ...
    //
    logger.error('JWT::generate error:', error);
    //
    throw error;
  }
}
...
export function parseAccessToken(token: string): IBaseJwtPayload | null {
  try {
